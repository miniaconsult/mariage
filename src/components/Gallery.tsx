...
export function Gallery() {
  ...
  const canShowNext =
    selectedIndex !== null && (selectedIndex < items.length - 1 || hasMore);
  ...
  }, [selectedIndex, items.length, hasMore, loadingMore]);
    ...
    <div className="w-full">
      ...
      {selected && (
        ...
        >
          ...
          {canShowPrev && (
            <button
              className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-black/40 p-3 font-sans text-2xl text-white/90 hover:bg-black/60 sm:left-4"
              onClick={(event) => {
          ...
          {canShowNext && (
            <button
              className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-black/40 p-3 font-sans text-2xl text-white/90 hover:bg-black/60 sm:right-4"
              onClick={(event) => {
                event.stopPropagation();
                showNext();
              }}
